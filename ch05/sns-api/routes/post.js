...
router.post('/', isLoggedIn, upload.single('img'), async (req, res) => {
   try {
      ...
      */
      ...
      //추출된 해시태그가 있으면
      if (hashtags) {
         //Promise.all: 여러개의 비동기 작업을 병렬로 처리. 모든 해시태그가 데이터 베이스에서 생성되거나 찾아질 때까지 기다림
         ...
         //findOrCreate: where절에서 찾는 값이 존재하는지 확인하고 없으면 create
...
router.put('/:id', isLoggedIn, upload.single('img'), async (req, res) => {
   try {
      ...
      const updatedPost = await Post.findOne({
         ...
         //users와 hashtags 테이블의 컬럼값을 포함해서 가져옴
         include: [
            {
               model: User,
...
router.get('/', async (req, res) => {
   try {
      ...
      select * from posts order by createdAt desc limit 3 offset
